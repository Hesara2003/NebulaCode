    ...
    if (workspaceId === 'demo-workspace') {
      ...
      for (const file of defaultFiles) {
        ...
        try {
          ...
          await this.saveFile(workspaceId, file.path, file.content);
        }
      }
    } else if (!workspaceExists) {
      // Default workspace - only create if workspace was just created
      await this.saveFile(
        workspaceId,
        'src/welcome.ts',
        `// Welcome to NebulaCode workspace: ${workspaceId}\nconsole.log("Hello World");`,
      );
    }
  }
  ...
  private async scanDirectory(
    ...
    for (const entry of entries) {
      ...
      const relativePath = path
        ...
        .replace(/\\/g, '/'); // Ensure forward slashes for IDs
